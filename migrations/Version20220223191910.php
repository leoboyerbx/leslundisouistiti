 ...
 */
final class Version20220223191910 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__application AS SELECT id, user_id, event_id, text, status FROM application');
        $this->addSql('DROP TABLE application');
        $this->addSql('CREATE TABLE application (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, event_id INTEGER NOT NULL, text CLOB NOT NULL COLLATE BINARY, status VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_A45BDDC1A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_A45BDDC171F7E88B FOREIGN KEY (event_id) REFERENCES event (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO application (id, user_id, event_id, text, status) SELECT id, user_id, event_id, text, status FROM __temp__application');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__event AS SELECT id, date, seats, title FROM event');
        $this->addSql('DROP TABLE event');
        $this->addSql('CREATE TABLE event (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, date DATE NOT NULL, seats INTEGER NOT NULL, title VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO event (id, date, seats, title) SELECT id, date, seats, title FROM __temp__event');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__application AS SELECT id, user_id, event_id, text, status FROM application');
        $this->addSql('DROP TABLE application');
        $this->addSql('CREATE TABLE application (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, event_id INTEGER NOT NULL, text CLOB NOT NULL, status VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO application (id, user_id, event_id, text, status) SELECT id, user_id, event_id, text, status FROM __temp__application');
        $this->addSql('DROP TABLE __temp__application');
        $this->addSql('CREATE INDEX IDX_A45BDDC1A76ED395 ON application (user_id)');
        $this->addSql('CREATE INDEX IDX_A45BDDC171F7E88B ON application (event_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__event AS SELECT id, date, seats, title FROM event');
        $this->addSql('DROP TABLE event');
        $this->addSql('CREATE TABLE event (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, date DATE NOT NULL, seats INTEGER NOT NULL, title VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO event (id, date, seats, title) SELECT id, date, seats, title FROM __temp__event');
        $this->addSql('DROP TABLE __temp__event');
        $this->addSql('DROP INDEX IDX_7CE748AA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__reset_password_request AS SELECT id, user_id, selector, hashed_token, requested_at, expires_at FROM reset_password_request');
